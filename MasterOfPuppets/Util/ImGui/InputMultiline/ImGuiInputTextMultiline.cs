...
using static MasterOfPuppets.Extensions.Dalamud.String.SeStringEx;
...
public class ImGuiInputTextMultiline
{
    ...
    private ImGuiInputTextDecorator textDecorator = new();
    ...
    private SeStringAutoCompletePopup AutoCompletePopup;
    ...
    {
        Plugin = plugin;
        AutoCompletePopup = new SeStringAutoCompletePopup(plugin);
    }
    ...
    )
    {
        ...
        ImGui.ImGuiInputTextCallbackDelegate decoratedCallback = (scoped ref ImGuiInputTextCallbackData data) =>
        {
            ...
            {
                var clipboardUtf8 = ClipboardExt.GetPayloadEnabledClipboardString();
            ...
            if (isCut || isCopy)
            {
                ...
                ClipboardExt.SetClipboardSeString(selectedSeString);
            }
    ...
    private IEnumerable<InputTextDecoration> GetDecorations(SeString input)
    {
        ...
        foreach (var payload in input.Payloads)
        {
            if (payload is AutoTranslatePayload atPayload)
            {
                ...
                yield return new InputTextDecoration.TextColor(startOffset, startOffset + 1, ImGui.ColorConvertFloat4ToU32(ColorPalette.AutoTranslateStartGreen));
                yield return new InputTextDecoration.TextColor(endOffset - 1, endOffset, ImGui.ColorConvertFloat4ToU32(ColorPalette.AutoTranslateEndRed));
                yield return new InputTextDecoration.HoverTooltip(startOffset, endOffset, new(() =>
                {
                    var completion = Plugin.CompletionIndex.ById(atPayload.Group, atPayload.Key);
