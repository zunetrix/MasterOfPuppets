...
// https://github.com/Haselnussbomber/HaselTweaks/blob/main/HaselTweaks/Windows/GearSetGridWindow.cs
public unsafe partial class GearSetGridWidget : Widget {
    public override string Title => "Gear Set Grid";
    ...
    public GearSetGridWidget(WidgetContext ctx) : base(ctx) {
    }

    // public GearSetGridWidget(Plugin plugin) : base($"{Plugin.Name}###GearSetGridWidget") {
    //     Plugin = plugin;
    ...
    public override void Draw() {
        ...
        using var table = ImRaii.Table("##TableGearsets", 1 + slotIndices.Length, ImGuiTableFlags.ScrollY | ImGuiTableFlags.NoSavedSettings);
        if (!table)
            return;
        ...
        for (var gearsetIndex = 0; gearsetIndex < gearsetCount; gearsetIndex++) {
            ...
            ImGui.TableNextColumn();
            {
                ...
                if (ImGui.IsItemHovered()) {
                    ...
                    if (gearset->GlamourSetLink != 0) {
                        ImGui.Text($"{TextService.GetAddonText(3185)}: {gearset->GlamourSetLink}"); // "Glamour Plate: {link}"
                    }
                }
                ...
                DalamudApi.TextureProvider.DrawIcon(62100 + gearset->ClassJob, new Vector2(iconSize));
            ...
            foreach (var slotIndex in slotIndices) {
                ...
                ImGui.SetCursorPosX(ImGui.GetCursorPosX() + IconSize.X * ImGuiHelpers.GlobalScale / 2f - ImGui.CalcTextSize(itemLevelText).X / 2f);

                // ImGui.TextColored(_itemService.GetItemLevelColor(item, gearset->ClassJob, Style.Colors.Red, Style.Colors.Yellow, Style.Colors.Green), itemLevelText);
    ...
    public void DrawItemIcon(GearsetEntry* gearset, GearsetItemIndex slotIndex, in GearsetItem slotItem, ItemHandle item) {
        ...
        DalamudApi.TextureProvider.DrawIcon(new GameIconLookup(GetItemIcon(item), ItemUtil.IsHighQuality(slotItem.ItemId)), (IconSize - IconInset * 2f) * ImGuiHelpers.GlobalScale);
        ...
        if (!ImGui.IsItemHovered() || !ExcelService.TryGetRow<Item>(item.ItemId, out var itemRow))
            return;

        using var _ = ImRaii.Tooltip();

        ImGui.TextColored(GetItemRarityColor(item), GetItemName(item).ToString());
        ...
        if (!glamourItem.IsEmpty) {
            ImGui.Text("GearSetGridWindow.ItemTooltip.LabelGlamour");
            ImGui.SameLine();
            ImGui.TextColored(GetItemRarityColor(glamourItem), GetItemName(glamourItem).ToString());
        ...
        if (slotItem.Stain0Id != 0 && ExcelService.TryGetRow<Stain>(slotItem.Stain0Id, out var stain0)) {
            ImGui.Text("GearSetGridWindow.ItemTooltip.LabelDye0");
            ImGui.SameLine();
            using (ImRaii.PushColor(ImGuiCol.Text, stain0.Color))
                ImGui.Bullet();
            ImGui.SameLine(0, 0);
            ImGui.Text(TextService.GetStainName(slotItem.Stain0Id));
        ...
        if (slotItem.Stain1Id != 0 && ExcelService.TryGetRow<Stain>(slotItem.Stain1Id, out var stain1)) {
            ImGui.Text("GearSetGridWindow.ItemTooltip.LabelDye1");
            ImGui.SameLine();
            using (ImRaii.PushColor(ImGuiCol.Text, stain1.Color))
                ImGui.Bullet();
            ImGui.SameLine(0, 0);
            ImGui.Text(TextService.GetStainName(slotItem.Stain1Id));
        ...
        if (usedInGearsets.Count > 1) {
            ImGui.Separator();
            ImGui.Text("GearSetGridWindow.ItemTooltip.AlsoUsedInTheseGearsets");
    ...
    public uint GetItemRarityColor(ItemHandle item, bool isEdgeColor = false) {
        var colorType = ItemUtil.GetItemRarityColorType(item, isEdgeColor);
        return colorType;
    }

    public ReadOnlySeString GetItemName(ItemHandle item, bool includeIcon = false) {
        return ItemUtil.GetItemName(item.ItemId, includeIcon, DalamudApi.ClientState.ClientLanguage);
    }

    public uint GetItemIcon(ItemHandle item) {
        return ItemHelper.GetIconId(item.ItemId);
    }
}
